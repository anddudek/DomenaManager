...
namespace DomenaManager.Helpers
{
   public static class PDFOperations
   {
      ...
      public static void AddTitle(Document document, string title, Building building)
      {
         Table buildingAddress = new Table();
         Column col = buildingAddress.AddColumn(Unit.FromCentimeter(8));
         col.Format.Alignment = ParagraphAlignment.Left;
         buildingAddress.Borders.Width = 1;
         buildingAddress.Borders.Color = Colors.Transparent;
         var buildingAddressRow = buildingAddress.AddRow();
         buildingAddressRow.Cells[0].AddParagraph(building.FullName);
         buildingAddressRow = buildingAddress.AddRow();
         buildingAddressRow.Cells[0].AddParagraph(building.GetAddress());
         document.LastSection.Add(buildingAddress);
      ...
      {
         Document doc = CreateTemplate(selectedCharge.Owner);
         AddTitle(doc, "Naliczenie z dnia: " + selectedCharge.ChargeDate.ToString("dd-MM-yyyy"), selectedCharge.Building);
         AddChargeTable(doc, selectedCharge, useDefaultFolder);
      }
      ...
      public static void PrepareSingleYearSummary(int year, Apartment apartment, Owner owner, Building building, bool useDefaultFolder)
      {
         ...
         AddTitle(doc, "Zestawienie roczne - " + year, building);
